...
run: build refresh-genesis-time
	@./bin/gean --genesis config.yaml --bootnodes nodes.yaml --validator-registry-path validators.yaml --validator-keys keys --node-id node0 --listen-addr /ip4/0.0.0.0/tcp/9003 --node-key node0.key --data-dir data/node0 --discovery-port 9003 --is-aggregator
