...
func mockProof(ids []uint64) *types.AggregatedSignatureProof {
	return &types.AggregatedSignatureProof{
		Participants: aggregation.AggregationBitsFromIndices(ids),
...
func TestCountParticipants(t *testing.T) {
	...
	empty := aggregation.AggregationBitsFromIndices([]uint64{})
...
func TestBuildBlock_RejectsUnclosedDivergence(t *testing.T) {
	...
	if !ok || se.Kind != store.ErrJustifiedDivergenceNotClosed {
		t.Fatalf("expected store.ErrJustifiedDivergenceNotClosed, got %v", err)
	}
}
