...
// EnvVar is the env var that gates the test-driver routes. When set to a
// truthy value, gean exposes POST /lean/v0/test_driver/* conformance endpoints.
...
// IsEnabled returns true if the env var is set to "1", "true",
// "TRUE", "yes", or "YES".
...
// NewSession returns an empty session; handlers will reject step
// calls until /fork_choice/init runs and populates store + fc.
func NewSession() *TestDriverSession {
	return &TestDriverSession{labelRoots: make(map[string][32]byte)}
}

// RegisterRoutes mounts the four test-driver endpoints on mux.
// Called only when HIVE_LEAN_TEST_DRIVER is truthy.
func RegisterRoutes(mux *http.ServeMux, sess *TestDriverSession) {
...
func (sess *TestDriverSession) applyAttestation(step *specfixtures.ForkChoiceStep) error {
	...
	participants := aggregation.AggregationBitsFromIndices([]uint64{step.Attestation.ValidatorID})
...
// verifyGossipAttestation is a thin wrapper around attestation.VerifyGossipAttestation
...
// refreshSafeTarget updates the snapshot after each processed step. The step
// handlers already populate fc.Votes.LatestNew via SetNew when attestations
// arrive, so this only recomputes and persists the value.
...
func TestDriverVerifySignaturesHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// blockprocessor.OnBlock runs full signature verification on the proposer and
		...
		if err := blockprocessor.OnBlock(store, signedBlock); err != nil {
			writeVerifyFailure(w, err.Error())
			return
		}
