...
func WriteBlockData(s *ConsensusStore, root [32]byte, signedBlock *types.SignedBlock) {
	...
	if err != nil {
		logger.Error(logger.Store, "write block data: begin write failed: %v", err)
		return
	}
...
func (s *ConsensusStore) InsertState(root [32]byte, state *types.State) {
	...
	wb, err := s.Backend.BeginWrite()
	if err != nil {
		logger.Error(logger.Store, "insert state: begin write failed: %v", err)
		return
	}
...
func (s *ConsensusStore) InsertBlockHeader(root [32]byte, header *types.BlockHeader) {
	...
	wb, err := s.Backend.BeginWrite()
	if err != nil {
		logger.Error(logger.Store, "insert block header: begin write failed: %v", err)
		return
	}
...
func (s *ConsensusStore) InsertLiveChainEntry(slot uint64, root, parentRoot [32]byte) {
	...
	wb, err := s.Backend.BeginWrite()
	if err != nil {
		logger.Error(logger.Store, "insert live chain entry: begin write failed: %v", err)
		return
	}
...
func (s *ConsensusStore) putMetadata(key, value []byte) {
	wb, err := s.Backend.BeginWrite()
	if err != nil {
		logger.Error(logger.Store, "put metadata: begin write failed: %v", err)
		return
	}
