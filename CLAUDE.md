...
A network with `EXECUTION_GENESIS_BLOCK_HASH` in `config.yaml` carries a Cancun `ExecutionPayload` in every `BlockBody` and caches its header in `State`; `statetransition.ProcessExecutionPayload` chains `parent_hash`, pins the timestamp to the slot, and requires the zero payload on networks without the key. `internal/execution/` defines the `Engine` interface using go-ethereum's own `beacon/engine` types (the only gean-side conversion is SSZ payload to executable data) with two implementations: `Client`, the remote Engine API client over JWT JSON-RPC, and `embedded.Engine`, geth linked in-process and driven through `catalyst.ConsensusAPI` (`--el-genesis`). A `Mock` serves tests, and `internal/node/execution_embedded_test.go` runs the driver against a real in-memory geth. `ExecutionDriver` on the `Engine` owns every call off the dispatch loop: build request at interval 4, `getPayload` in the proposal worker, `newPayload` on a serial ingress worker in front of `BlockCh` (only VALID imports; unresolved blocks wait in a bounded retry queue with missing-parent recovery and requested-block backpressure), and forkchoice updates on head change and once per slot. Withdrawals and blob transactions are rejected by consensus and the Engine API client, and the demo config disables blobs. Proposals validate before signing, and restored heads require execution confirmation before validator duties. Genesis seeds the header and body from the config key, and startup refuses an execution client whose block 0 differs.
