...
Run everything here on the user's devnet server (ask for the host if unknown),
from the gean repo root, where `make lean-quickstart` cloned `lean-quickstart/`.

gean logs carry ANSI color codes, including a reset between `[component]` and
the message. Pipe logs through `sed -E 's/\x1b\[[0-9;]*m//g'` before grepping.
...
for a 5-client setup (zeam, ream, lantern, ethlambda, gean). Take peer images
from `lean-quickstart/client-cmds/<client>-cmd.sh` (see `clients.md`).

The server may host other devnets. Before starting, confirm none of the
container names below exist (`docker ps -a`); if one does, it is not yours —
pick different names rather than removing it. Every `docker rm -f` in this
guide targets only containers you started.

```bash
GENESIS=/path/to/lean-quickstart/local-devnet/genesis
DATA=/path/to/lean-quickstart/local-devnet/data
GEAN_IMAGE=ghcr.io/geanlabs/gean:devnet5   # from make docker-build
ZEAM_IMAGE=...       # node_docker in client-cmds/zeam-cmd.sh
REAM_IMAGE=...       # node_docker in client-cmds/ream-cmd.sh
LANTERN_IMAGE=...    # node_docker in client-cmds/lantern-cmd.sh
ETHLAMBDA_IMAGE=...  # node_docker in client-cmds/ethlambda-cmd.sh

for n in zeam_0 ream_0 lantern_0 ethlambda_0 gean_0; do
  docker ps -aq --filter "name=^/$n\$" | grep -q . && { echo "$n exists"; exit 1; }
done
...
for n in zeam_0 ream_0 lantern_0 ethlambda_0 gean_0; do
  printf "$n: "
  docker logs --tail 200 "$n" 2>&1 | sed -E 's/\x1b\[[0-9;]*m//g' | grep -i "finalized" | tail -1
done
```

All nodes should show the same finalized slot advancing. gean logs
`[forkchoice] finalized advanced slot=N` on each advance and
`finalized_slot=N` on each processed block.
...
GENESIS=/path/to/lean-quickstart/local-devnet/genesis
DATA=/path/to/lean-quickstart/local-devnet/data
NEW_IMAGE=gean:my-new-tag   # must not be the tag the running container uses
...
# 3. Stop and remove the container (one you started)
docker rm -f gean_0
rm -rf "$DATA/gean_0/"*
...
docker logs --tail 200 gean_0 2>&1 | strip | grep "\[gossip\] received block"

# Check finalization matches other nodes
for n in zeam_0 ream_0 lantern_0 ethlambda_0 gean_0; do
  printf "$n: "
  docker logs --tail 200 "$n" 2>&1 | strip | grep -i "finalized" | tail -1
done
```

**Only proceed to the next node after confirming:**
- The restarted node logs `[gossip] received block` for new slots
...
### Restarted node never logs `[gossip] received block`

It has not joined the gossip mesh. The 60-second wait was not long enough, or was skipped. Stop the node, wait
60s, and start again.
...
### Checkpoint fetch fails (connection refused)
...
If the existing container is one you started and meant to replace, remove it
with `docker rm -f <name>`. Otherwise it belongs to someone else; use a
different name.
