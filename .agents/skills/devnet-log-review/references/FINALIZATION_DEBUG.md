...
- a ≥ 2/3 supermajority of validators attesting: `3 * votes >= 2 * total`
  (`internal/statetransition/attestations.go`)
...
```bash
# Last head event per node (head, justified, finalized)
.agents/skills/devnet-log-review/scripts/check-consensus-progress.sh

# gean
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "finalized advanced slot=" | tail -20
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "finalization stalled:"
...
How often finalization advances depends on configuration (validator count,
slot timing), participation, and the 3SF-mini gap rule; there is no fixed
cadence. Judge it against head progress in the same run.

**Stall indicator:** Finalized slot stays the same for 50+ slots while head
slot continues advancing.

## Example of Healthy Finalization (illustrative)
...
**Requirement:** Need a **≥ 2/3 supermajority** to justify
...
```bash
# gean
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "attestation channel full"

# ethlambda (unverified)
...
- May prevent reaching the 2/3 threshold even if enough validators are on canonical fork
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "Latest Justified:\|justified_slot=" | tail -30
...
```bash
# One summary per aggregation session; produced=0 with skipped=... says why
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "aggregation worker: slot=" | tail -20
...
for slot in 28 29 30 31 32; do
  echo "=== Slot $slot ==="
  sed -E 's/\x1b\[[0-9;]*m//g' *.log | grep -h "slot=$slot[^0-9]" | grep -oE "0x[a-f0-9]{8}" | sort -u
done
```
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "attestation verified.*slot=30" | wc -l

# Expected: roughly one per validator on other nodes (every gean validator attests)
```
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "justified_slot=" | grep -oE "justified_slot=[0-9]+" | sort -u
