...
Guide to identifying and analyzing forks in devnet runs. gean patterns are
verified against source; other clients' patterns are unverified (see
[CLIENT_LOG_PATTERNS.md](CLIENT_LOG_PATTERNS.md)).
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "\[chain\] block slot=" | head -20
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "\[gossip\] received block" | head -20

# When parent is missing, gean logs and stores as pending
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "block parent missing"
...
```bash
sed -E 's/\x1b\[[0-9;]*m//g' zeam_0.log | grep "processing block\|imported block\|processed block" | head -20
```
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "\[chain\] block slot=" | \
...
# gean
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "block processing failed"
...
for f in *.log; do
    node=$(basename "$f" .log)
    head_line=$(sed -E 's/\x1b\[[0-9;]*m//g' "$f" | grep -E "Head Block Root|Head Slot" | tail -1)
    echo "$node: $head_line"
done
```
...
# Compare finalization across nodes
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "produced attestation\|proposing block" | head -3
...
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep "REORG"
# Output: REORG depth=D slot=N head_root=0x... parent_root=0x... (was 0x...) justified_slot=J ... finalized_slot=F ...

# Count reorgs per node
sed -E 's/\x1b\[[0-9;]*m//g' gean_0.log | grep -c "REORG"
```
