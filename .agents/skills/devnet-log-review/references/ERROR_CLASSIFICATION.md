...
Reference for categorizing and understanding errors in devnet logs. Patterns
marked (gean) are verified against gean's source; the rest come from other
clients' past logs and are unverified.
...
`count-errors-warnings.sh` excludes only `NoFinalizedStateFound` and
`HandshakeTimedOut` from error counts and `TODO` from warning counts; the
scripts skip `devnet.log`, where Docker's `manifest unknown` appears.
...
| `block parent missing slot=N ... depth=D, storing as pending` (gean, WARN) | gean's pending block cache absorbing orphan | Normal during sync; problematic if depth grows |
...
Example from ethlambda logs (unverified):
```
...
- `[forkchoice] finalized advanced slot=` recurs
...
|---|---|
| `[chain] block processing failed` | Invalid block or state transition bug |
| `[store] finalization stalled:` | Finalization not advancing; see FINALIZATION_DEBUG.md |
...
```bash
# Error details per node (skips devnet.log)
.agents/skills/devnet-log-review/scripts/show-errors.sh -l 50
...
grep "block processing failed" gean_0.log                 # gean
grep "Failed to process block" ethlambda_0.log            # unverified
grep "signature verification failed" lantern_0.log        # unverified
grep "Failed to handle process" ream_0.log                # unverified
```
