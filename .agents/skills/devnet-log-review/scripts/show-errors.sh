...
#   -n NODE   Filter to specific node (e.g., "gean_0")
...
for f in "${log_files[@]}"; do
    ...
    # Show errors
    # grep exits 1 on no match and head may SIGPIPE grep; neither is an error here.
    error_count=$(strip_ansi < "$f" | grep -ci "error" || true)
    echo "Errors ($error_count total, showing first $limit):"
    strip_ansi < "$f" | grep -i "error" | head -"$limit" || true

    # Optionally show warnings
    if $show_warnings; then
        echo ""
        warning_count=$(strip_ansi < "$f" | grep -ci "warn" || true)
        echo "Warnings ($warning_count total, showing first $limit):"
        strip_ansi < "$f" | grep -i "warn" | head -"$limit" || true
    fi
