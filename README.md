...
### Running with an execution layer

A network declares an execution layer by carrying the execution client's
block 0 hash in `config.yaml` as `EXECUTION_GENESIS_BLOCK_HASH`. Every block
on such a network embeds an execution payload: the proposer fetches it from
its execution client over the Engine API, and importers have their own
