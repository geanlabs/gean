...
func NewStore(state *types.State, anchorBlock *types.Block, store storage.Store) *Store {
	...
	return &Store{
		...
		LatestJustified:         &types.Checkpoint{Root: anchorRoot, Slot: state.LatestJustified.Slot},
		LatestFinalized:         &types.Checkpoint{Root: anchorRoot, Slot: state.LatestFinalized.Slot},
		Storage:                 store,
